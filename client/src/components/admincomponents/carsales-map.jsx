...
const CarAuction = ({ car, getCarDetails, backendURL }) => {
  ...
  const carLotData =
    car.sellingType === "auction"
      ...
      : { auctionLot: "" };
  const carLotNumber =
    car.sellingType === "auction" ? { lotNo: car.lotNo || "" } : { lotNo: "" };
  ...
  const nextCar = async () => {
    try {
      ...
      const response = await fetch(
        `${backendURL}/car/unsold/${carLotData.auctionLot}/${carLotNumber.lotNo}`,
        {
          method: "GET",
        }
      );
      ...
      <div className="car-auction">
        ...
        {car.sellingType === "auction" ? (
          <p className="lot-info">
            {" "}
            Lot No. {car.lotNo} | Model: {car.carModal || "No Model"}
          </p>
        ) : (
          <p className="lot-info">
            {" "}
            VIN. {car.vin} | Model: {car.carModal || "No Model"}
          </p>
        )}
        ...
        <div className="current-bid">
          {car.sellingType === "auction" ? (
            <>
              <button style={{ float: "right" }} onClick={comingNext}>
                Next <ArrowBigRight />
              </button>
            ...
            <>
              ...
              <h2>
                AED{" "}
                {car.discountedPrice ? car.discountedPrice : car.price || "N/A"}
              </h2>
              {car.discountedPrice && (
                <p>Original Price: {car.price || "N/A"} AED</p>
              )}
            </>
          )}
        </div>
        {!car.isSold ? (
                    ...
                    <div className="bid-controls">
                      ...
                      currentBidData?.carId === car._id ? (
                        <button
                          ...
                          disabled={bidLoading}
                          style={{
                            backgroundColor: bidLoading ? "gray" : "#405FF2",
                          }}
                        >
                  ...
                  <div className="form-grid">
                    <div className="input-container" id="auction-container">
                      ...
                      <button
                        ...
                        onClick={handlePriceColorChange}
                      >
                        Update
                      </button>
                    </div>
                  ...
                  <div className="form-grid">
                    <div className="input-container" id="auction-container">
                      <Select
                        ...
                        value={generateAuctionOptions().find(
                          (option) => option.value === selectedAuctionLot
                        )} // Match the selected value
                        onChange={(selectedOption) =>
                          setSelectedAuctionLot(selectedOption?.value)
          ...
          <div className="bid-controls">
            <button className="place-bid">Car has been already Sold</button>
          </div>
        )}
